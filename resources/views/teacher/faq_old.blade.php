    ...
    <div class="row">
        <div class="col-lg-4">
            <div class="nav nav-pills faq-nav" id="faq-tabs" role="tablist" aria-orientation="vertical">
                ...
                <a href="#tab_{{$category}}" class="nav-link active" data-toggle="pill" role="tab" aria-controls="tab_{{$category}}" aria-selected="false">
                    {{$category}}
                </a>
        ...
        <div class="col-lg-8">
            <div class="tab-content" id="faq-tab-content">
                @foreach($categories as $category)
                    <div class="tab-pane show active" id="tab_{{$category}}" role="tabpanel" aria-labelledby="tab_{{$category}}">
                        <div class="accordion" id="accordion-tab-{{$category}}">
                            @foreach ($faqs->where('faq_category',$category) as $faq)
                                <div class="card">
                                    <div class="card-header" id="accordion-tab-{{$category}}-question-{{$faq->id}}">
                                        <h5><button class="btn btn-link" type="button" data-toggle="collapse" data-target="#accordion-tab-{{$category}}-answer-{{$faq->id}}" aria-expanded="false" aria-controls="accordion-tab-{{$category}}-answer-{{$faq->id}}">{{$faq->faq_question}}</button></h5>
                                    </div>
                                    <div class="collapse show" id="accordion-tab-{{$category}}-answer-{{$faq->id}}" aria-labelledby="accordion-tab-{{$category}}-question-{{$faq->id}}" data-parent="#accordion-tab-{{$category}}">
                                        <div class="card-body">
                                            <p>{{$faq->faq_answer}}</p>
                                        </div>
                                    </div>
